...
public sealed class TokenService(IOptions<JwtOptions> options)
{
    public string CreateAdministrator(Guid administratorId, string username)
    {
        ...
        var token = new JwtSecurityToken(settings.Issuer, settings.Audience,
            [new Claim(JwtRegisteredClaimNames.Sub, administratorId.ToString()), new Claim("admin_username", username), new Claim(PortalSecurity.AdminClaim, "true"), new Claim(PortalSecurity.RoleClaim, PortalSecurity.AdministratorRole)],
    ...
    {
        var settings = options.Value;
        var claims = new[] { new Claim(JwtRegisteredClaimNames.Sub, subject), new Claim("tenant_id", tenantId.ToString()), new Claim(PortalSecurity.RoleClaim, PortalSecurity.UserRole) };
