...
import { AlertNotification, AlertRepeatMode, AlertRule } from '../core/models';
import { errorMessage } from '../core/api';
import { IconComponent } from '../shared/icon.component';

interface RuleForm {
  name: string; provider: string; status: number; threshold: number; windowMinutes: number;
  repeatMode: AlertRepeatMode; repeatIntervalMinutes: number; isActive: boolean;
}

@Component({ selector: 'sms-alerts', imports: [DatePipe, FormsModule, IconComponent], template: `
<div class="page-heading"><div><span class="eyebrow">PROACTIVE MONITORING</span><h1>Alerts<span class="heading-dot">.</span></h1><p>Create rules based on message volume, status, time window, and provider.</p></div><button class="button compact" (click)="load()" [disabled]="loading()"><sms-icon name="refresh"/>Refresh</button></div>
...
<section class="panel padded create-panel">
  ...
  <form (ngSubmit)="save()">
    <div class="form-grid">
      ...
      <div><label for="rule-threshold">Message quantity</label><input id="rule-threshold" name="threshold" type="number" min="1" max="1000000" required [(ngModel)]="form.threshold"/></div>
      <div><label for="rule-window">Time window (minutes)</label><input id="rule-window" name="windowMinutes" type="number" min="1" max="43200" required [(ngModel)]="form.windowMinutes"/></div>
      <div><label for="rule-repeat">Alert frequency</label><select id="rule-repeat" name="repeatMode" [(ngModel)]="form.repeatMode"><option [ngValue]="1">Once per incident</option><option [ngValue]="2">Repeat at an interval</option></select></div>
      @if (form.repeatMode === 2) { <div><label for="rule-repeat-interval">Repeat interval (minutes)</label><input id="rule-repeat-interval" name="repeatIntervalMinutes" type="number" min="1" max="43200" required [(ngModel)]="form.repeatIntervalMinutes"/></div> }
    </div>
...
<section class="panel create-panel">
  <div class="panel-heading"><div><h2>Rules</h2><p>Each rule is evaluated independently every minute.</p></div></div>
  <div class="table-scroll"><table><thead><tr><th>Name</th><th>Condition</th><th>Frequency</th><th>State</th><th>Actions</th></tr></thead><tbody>
  @for (rule of rules(); track rule.id) { <tr><td><strong>{{ rule.name }}</strong><small class="cell-secondary">{{ rule.provider || 'All providers' }}</small></td><td>{{ rule.threshold }} {{ statusLabel(rule.status).toLowerCase() }} in {{ rule.windowMinutes }} min</td><td>{{ rule.repeatMode === 1 ? 'Once per incident' : 'Every ' + rule.repeatIntervalMinutes + ' min' }}</td><td><span class="badge" [class.success]="rule.isActive" [class.warning]="rule.isTriggered">{{ !rule.isActive ? 'Disabled' : rule.isTriggered ? 'Triggered' : 'Monitoring' }}</span></td><td><div class="button-row"><button class="button compact" (click)="edit(rule)">Edit</button><button class="button compact destructive" (click)="remove(rule)">Delete</button></div></td></tr> }
  @empty { <tr><td colspan="5" class="empty-cell">No alert rules configured.</td></tr> }
  </tbody></table></div>
</section>
...
export class AlertsComponent {
  ...
  save() {
    ...
    const payload = { ...this.form, provider: this.form.provider.trim() || null, repeatIntervalMinutes: this.form.repeatMode === 2 ? this.form.repeatIntervalMinutes : null };
  ...
  edit(rule: AlertRule) {
    ...
    this.form = { name: rule.name, provider: rule.provider || '', status: rule.status, threshold: rule.threshold, windowMinutes: rule.windowMinutes, repeatMode: rule.repeatMode, repeatIntervalMinutes: rule.repeatIntervalMinutes || 15, isActive: rule.isActive };
  ...
  private emptyForm(): RuleForm { return { name: '', provider: '', status: 4, threshold: 10, windowMinutes: 15, repeatMode: 1, repeatIntervalMinutes: 15, isActive: true }; }
}
