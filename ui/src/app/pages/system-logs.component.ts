...
export class SystemLogsComponent {
  ...
  readonly logs = signal<LogEntry[]>([]); readonly loading = signal(false); readonly error = signal(''); readonly page = signal(0); readonly hasNext = signal(false); private readonly cursors: ({ timestamp: string; id: number } | null)[] = [null];
