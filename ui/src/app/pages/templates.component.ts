  ...
  template: `
    ...
    @if (editing) {
      <section class="card">
        ...
        <div class="actions"><button (click)="getAiTips()" [disabled]="aiBusy || !editing.body.trim()">AI tips</button><button class="primary" (click)="save()">Save template</button><button (click)="editing = null">Cancel</button></div>
        @if (aiResult) { <div class="notice" role="status"><strong>AI suggestions</strong><p>{{ aiResult }}</p></div> }
...
export class TemplatesComponent implements OnInit {
  ...
  getAiTips() { this.runAi(); }
  private runAi() {
    if (!this.editing?.body.trim() || this.aiBusy) return;
    this.aiBusy = true; this.aiResult = ''; this.error = '';
    this.http.post<{ message: string; issues: string[]; isValid: boolean }>('/api/v1/message-assistant/validate', { message: this.editing.body }).subscribe({
      next: result => { this.aiBusy = false; this.aiResult = result.isValid ? 'No improvements suggested.' : result.issues.join(' · '); },
