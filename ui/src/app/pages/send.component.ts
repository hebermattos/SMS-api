...
export class SendComponent {
  ...
  private runAi() {
    ...
    this.http.post<{ message: string; issues: string[]; isValid: boolean }>('/api/v1/message-assistant/validate', { message: this.body })
      .pipe(takeUntilDestroyed(this.destroyRef), finalize(() => this.aiBusy.set(false)))
      .subscribe({ next: result => this.aiResult.set(result.isValid ? 'No improvements suggested.' : result.issues.join(' · ')), error: error => this.error.set(errorMessage(error)) });
  }
