...
@Component({ selector: 'sms-shell', imports: [RouterLink, RouterLinkActive, RouterOutlet, IconComponent], template: `
  ...
  <div class="workspace">
    ...
    <aside class="sidebar" [class.open]="menuOpen()"><a class="brand" [routerLink]="auth.role() === 'admin' ? '/admin/tenants' : '/app'"><span class="brand-mark"><sms-icon name="message"/></span>SMS<span class="brand-light">console</span></a>
      ...
      <div class="sidebar-bottom"><div class="sidebar-note"><sms-icon name="shield"/><strong>{{ auth.role() === 'admin' ? 'Management with control' : 'Your workspace, your data' }}</strong><p>{{ auth.role() === 'admin' ? 'Manage companies and credentials in a separate environment.' : 'The information in this console belongs only to your company.' }}</p></div><button class="logout" (click)="auth.logout()"><sms-icon name="logout"/>Sign out</button></div>
    </aside>
    <div class="workspace-main"><header class="topbar"><div class="topbar-left"><button class="icon-button mobile-menu" aria-label="Open menu" [attr.aria-expanded]="menuOpen()" (click)="menuOpen.set(!menuOpen())"><sms-icon name="menu"/></button><span class="breadcrumb">Console <span>/</span> {{ auth.role() === 'admin' ? 'Administration' : 'Workspace' }}</span></div><div class="account"><span class="badge">{{ auth.role() === 'admin' ? 'Administrator' : 'Tenant' }}</span><span class="account-name">{{ auth.identity() }}</span><span class="avatar">{{ auth.identity().slice(0, 1).toUpperCase() }}</span></div></header>
      <main class="page-content" id="main-content" tabindex="-1"><router-outlet/></main>
      <footer class="workspace-footer"><span>SMS Console</span><span>Communication, made clear.</span></footer>
    </div>
  </div>` })
