...
The following diagram represents the PostgreSQL application schema defined in `database/schema.sql`. Auxiliary observability and reporting databases are kept separate from the transactional application database.

```mermaid
erDiagram
    Tenants ||--o{ PortalUsers : has
    Tenants ||--o| TenantRateLimits : configures
    Tenants ||--o| TenantAiSettings : configures
    Tenants ||--o{ MessageTemplates : owns
    Tenants ||--o{ SmsMessages : owns
    PortalUsers o|--o{ SmsMessages : sends
    Tenants ||--o{ SmsOptOuts : owns
    Tenants ||--o{ TenantSmsProviders : configures
    Tenants ||--o{ ApiClients : owns
    SmsMessages ||--o{ SmsMessageStatusHistory : has
    Tenants ||--o{ AlertRules : owns
    AlertRules ||--o{ Alerts : triggers
    Tenants ||--o{ AlertStatusCounters : aggregates
    Tenants ||--o{ AlertEvaluationOutbox : owns
    Tenants ||--o{ TenantSmsOverviewOutbox : owns
    ...
    AlertStatusCounters {
        ...
        varchar Provider PK
        int Status PK
        timestamptz BucketStartUtc PK
        int MessageCount
    }
    AlertEvaluationOutbox {
        uuid Id PK
        uuid TenantId FK
        varchar Provider
        int Status
    ...
    AlertEvaluationInbox {
        uuid EventId PK
        timestamptz ProcessedAtUtc
    }
    TenantSmsOverviewOutbox {
...
`PlatformAdministrators` and `AlertEvaluationInbox` are intentionally shown without relationships because the current schema does not define foreign keys for them.
