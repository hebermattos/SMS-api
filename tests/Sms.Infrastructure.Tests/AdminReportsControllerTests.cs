...
public sealed class AdminReportsControllerTests
{
    ...
    public async Task Sms_PassesFiltersAndNormalizesProvider()
    {
        ...
        Assert.Equal(SmsStatus.Delivered, repository.Filter.Status);
        Assert.Equal(SmsDirection.Outbound, repository.Filter.Direction);
    ...
    private sealed class Reports : ISmsReportRepository
    {
        ...
        { Filter = filter; return Task.FromResult(new PlatformSmsReportSummary(0,0,0,0,0,0,0,[])); }
